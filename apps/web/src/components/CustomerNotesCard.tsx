 ...
 * and employee can add one — employees are the ones actually taking the
 * call/text and logging what happened, not just admins.
 */
export function CustomerNotesCard({ customerId }: { customerId: string }) {
  const { data: currentUser } = useCurrentUser();
  const canEdit = currentUser?.user?.role === "admin" || currentUser?.user?.role === "employee";
