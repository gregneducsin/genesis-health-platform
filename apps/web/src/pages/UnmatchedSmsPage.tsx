...
import type { UnmatchedSmsThreadSummary } from "@luma/shared";
...
function ThreadMessages({ threadId }: { threadId: string }) {
  const { data, isLoading } = useUnmatchedSmsThread(threadId);
...
function ThreadRow({ thread }: { thread: UnmatchedSmsThreadSummary }) {
  const [expanded, setExpanded] = useState(false);
  const [draft, setDraft] = useState(thread.suggestedReply ?? "");
  const sendReply = useSendUnmatchedSmsReply();
  const dismiss = useDismissUnmatchedSms();
      ...
      {expanded && (
        <div className="space-y-3 border-t border-gray-100">
          <div className="px-4 pt-3">
            ...
            <p className="text-sm text-gray-800">{thread.fromName ? `${thread.fromName} (${thread.fromPhone})` : thread.fromPhone}</p>
            {thread.collectedEmail && <p className="mt-0.5 text-xs text-gray-500">{thread.collectedEmail}</p>}
          </div>
          ...
          {thread.linkedCustomerId && (
            <div className="mx-4 rounded-md bg-green-50 px-3 py-2">
              <p className="text-xs text-green-800">
                This sender is now linked to{" "}
                <a href={`/customers/${thread.linkedCustomerId}`} className="font-medium underline">
                  a customer record
                </a>{" "}
                — either a new lead created from their name and email, or an existing customer auto-connected because both matched. See their
                customer record or the Conversations/Support tab for what Joy or Lisa said next. Every text from this number from now on routes
                straight to them automatically.
              </p>
            </div>
          )}
...
export function UnmatchedSmsPage() {
  const { data, isLoading } = useUnmatchedSmsList();
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-gray-900">Unmatched Texts</h1>
        {data && <p className="text-sm text-gray-500">{needsReview.length} awaiting review</p>}
      </div>
      <p className="text-xs text-gray-400">
        Inbound text from a phone number that doesn't match any customer record, grouped one thread per number. Replies are auto-sent by default:
        a fixed acknowledgment goes out on the first message (asking for their name), and Claude's own drafted reply goes out on every message
        after that. Threads only land here for review when Claude flags genuine uncertainty (or an individualized medical/suitability question),
        or when the sender looks like a possible match to an existing customer, or claims to already have an account — those never auto-link or
        auto-send, except when the texted name and a collected email both point at the exact same existing customer (or the sender explicitly
        confirms it), in which case they're connected straight to their real conversation automatically. Once we know their name and email and
        Claude is confident it's a genuine new lead, a lead is created automatically and the message is handed straight to Joy's real pipeline —
        she takes it from there, same as any other lead.
      </p>
