...
describe("GET /api/app/failed-payments", () => {
  ...
  it("rejects employee", async () => {
    await seedUser("failedpay-emp1@example.com", "employee");
    const emp = await loginAgent(app, "failedpay-emp1@example.com");
    expect((await emp.agent.get("/api/app/failed-payments")).status).toBe(403);
  });
...
describe("POST /api/app/failed-payments/:id/resolve", () => {
  ...
  it("rejects manager and employee from mutating", async () => {
    const id = await seedFailedPayment();
    await seedUser("failedpay-emp2@example.com", "employee");
    const emp = await loginAgent(app, "failedpay-emp2@example.com");
    const res = await emp.agent.post(`/api/app/failed-payments/${id}/resolve`).set("x-csrf-token", emp.csrf).send({});
