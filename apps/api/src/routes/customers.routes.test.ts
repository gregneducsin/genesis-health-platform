...
describe("Customers CRUD", () => {
  ...
  it("rejects employee role from the Leads list, but allows reading a single customer — this powers the identity-confirmation link on Unmatched Contacts", async () => {
    const { db, customersTable } = await import("@luma/db");
    const [customer] = await db
      .insert(customersTable)
      .values({ firstName: "Emp", lastName: "Target", email: "emp-target@example.com", leadReceivedDate: "2026-01-01" })
      .returning();
    ...
    expect(detailRes.body.customer.email).toBe("emp-target@example.com");
  });
...
describe("Upcoming trigger", () => {
  ...
  it("rejects employee on both the read and the cancel — employee can't reach Conversations/Support", async () => {
    await seedUser("trigger-emp1@example.com", "employee");
    const { agent, csrf } = await loginAgent(app, "trigger-emp1@example.com");
...
describe("Customer notes", () => {
  ...
  it("employee can both read and add notes — this is who actually takes the call/text and logs it", async () => {
    ...
    await seedUser("notes-emp1@example.com", "employee");
    const employee = await loginAgent(app, "notes-emp1@example.com");
    const addRes = await employee.agent
    ...
    expect(addRes.body.note).toMatchObject({ customerId, authorEmail: "notes-emp1@example.com" });

    const listRes = await employee.agent.get(`/api/app/customers/${customerId}/notes`);
    expect(listRes.status).toBe(200);
    expect(listRes.body.notes).toHaveLength(1);
  });
