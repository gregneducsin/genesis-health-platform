...
vi.mock("../lib/email-provider.js", async () => {
  ...
  return { ...actual, getEmailProvider: () => ({ provider: { sendEmail: sendEmailMock }, fromName: "Genesis Health" }) };
});
...
describe("Users", () => {
  ...
  it("rejects manager and employee roles — user management is admin-only", async () => {
    ...
    await seedUser("users-emp@example.com", "employee");
    const employee = await loginAgent(app, "users-emp@example.com");
    expect((await employee.agent.get("/api/app/users")).status).toBe(403);
    expect(
      (await employee.agent.patch(`/api/app/users/${target.id}`).set("x-csrf-token", employee.csrf).send({ role: "admin" })).status,
    ).toBe(403);
  });
  ...
  it("admin invites a new user: creates the row, sends the invite email, and lists it", async () => {
    ...
    const res = await agent
      ...
      .send({ email: "newhire@example.com", firstName: "New", lastName: "Hire", role: "employee" });
    expect(res.status).toBe(201);
    expect(res.body.user).toMatchObject({ email: "newhire@example.com", role: "employee", status: "invited" });
  ...
  it("still creates the invite even when the email send fails", async () => {
    ...
    const res = await agent
      .post("/api/app/users")
      .set("x-csrf-token", csrf)
      .send({ email: "email-fails@example.com", firstName: "A", lastName: "B", role: "employee" });
    expect(res.status).toBe(201);
  ...
  it("admin changes a user's role, but can't target themselves or a nonexistent user", async () => {
    ...
    const target = await seedUser("users-target1@example.com", "employee");
  ...
  it("disabling a user revokes their existing session immediately; reactivating lets them log in again", async () => {
    await seedUser("users-target2@example.com", "employee");
