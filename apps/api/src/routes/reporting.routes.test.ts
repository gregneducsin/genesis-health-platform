...
describe("GET /api/app/reporting/funnel", () => {
  ...
  it("rejects employee", async () => {
    await seedUser("reporting-emp1@example.com", "employee");
    const employee = await loginAgent(app, "reporting-emp1@example.com");
    expect((await employee.agent.get("/api/app/reporting/funnel")).status).toBe(403);
  });
