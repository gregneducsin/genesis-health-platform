...
describe("GET /api/app/webhook-events", () => {
  ...
  it("rejects employee", async () => {
    await seedUser("webhookevt-emp1@example.com", "employee");
    const employee = await loginAgent(app, "webhookevt-emp1@example.com");
    expect((await employee.agent.get("/api/app/webhook-events")).status).toBe(403);
  });
