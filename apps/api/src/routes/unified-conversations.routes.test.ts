...
describe("GET /api/app/conversations (unified)", () => {
  ...
  it("allows admin and employee", async () => {
    ...
    await seedUser("unifiedconvo-emp1@example.com", "employee");
    const emp = await loginAgent(app, "unifiedconvo-emp1@example.com");
    expect((await emp.agent.get("/api/app/conversations")).status).toBe(200);
  });
