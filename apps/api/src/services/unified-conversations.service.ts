...
import { sendEmailStaffReply as sendChrisEmailStaffReply } from "./chris-email-dispatch.service.js";
import { sendEmailStaffReply as sendMiaEmailStaffReply } from "./mia-email-dispatch.service.js";
...
export async function sendUnifiedStaffReply(
  ...
  if (persona === "sales" && channel === "email") {
    ...
    if (!row) return { sent: false, reason: "not_found" };
    return sendChrisEmailStaffReply(row.id, body, staffEmail);
  }
  ...
  if (!row) return { sent: false, reason: "not_found" };
  return sendMiaEmailStaffReply(row.id, body, staffEmail);
}
