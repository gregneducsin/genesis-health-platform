...
/**
 * Same fail-soft send+log pattern as Chris's dispatch (chris-dispatch.service.ts's
...
async function sendAndLog(personId: string, conversationId: string, phone: string | null, text: string): Promise<void> {
  if (await isCustomerSmsDnd(personId)) {
    logger.warn({ personId, conversationId }, "outbound Mia message not sent: customer is do-not-disturb");
    return;
  }
  ...
  if (phone) {
    try {
      ...
      logger.warn({ conversationId, reason: err instanceof Error ? err.message : String(err) }, "outbound Mia message send failed");
    }
  } else {
    logger.warn({ conversationId }, "outbound Mia message not sent: no phone number on file");
  }
  await appendSupportMessage(conversationId, "outbound", text, { providerMessageId });
}

/**
 * Full inbound-turn pipeline for Mia, mirroring processInboundMessage in
 * chris-dispatch.service.ts: persist the inbound message, run the guardrail
 * loop, tag sentiment, send+log the validated reply, persist updated state.
 *
 * Wrapped in withPersonLock for the same reason as Chris's dispatch — see
 * the comment there — so a patient double-texting can't race two turns
 * into clobbering each other's conversation-state write.
 */
export async function processInboundSupportMessage(personId: string, inboundBody: string): Promise<MiaTurnResult> {
  return withPersonLock(personId, () => processInboundSupportMessageLocked(personId, inboundBody));
}

async function processInboundSupportMessageLocked(personId: string, inboundBody: string): Promise<MiaTurnResult> {
  ...
  const body = toMiaPreviewBody(conversation, [...priorMessages, inboundMessage]);
  let result: MiaTurnResult;
  try {
    result = await runMiaTurn(body);
  } catch (err) {
    // Same reasoning as chris-dispatch.service.ts's equivalent catch: anything
    // that escapes runMiaTurn itself isn't a guardrail rejection, but the
    // patient still got silence, so it needs the same staff-visible flag.
    logger.error({ personId, conversationId: conversation.id, reason: err instanceof Error ? err.message : String(err) }, "Mia turn threw unexpectedly — no outbound message sent");
  ...
  if (!result.ok) {
    logger.warn({ personId, conversationId: conversation.id, code: result.code }, "Mia turn rejected — no outbound message sent");
  ...
  // comment in chris-dispatch.service.ts's processInboundMessageLocked.
