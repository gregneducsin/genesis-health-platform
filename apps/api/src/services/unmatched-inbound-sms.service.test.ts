...
describe("recordAndClassifyUnmatchedSms", () => {
  ...
  it("grounds the triage model in what Genesis actually sells, so it can't invent services when asked what the business offers", async () => {
    createMock.mockResolvedValueOnce(toolResponse(classification()));
    await recordAndClassifyUnmatchedSms(uniquePhone(), "what does genesis health offer?");
    ...
    expect(systemPromptArg).toContain("Never invent services, product categories, or");
  });
  ...
  it("holds the reply for human review when Claude self-reports mentioning an out-of-scope business line, even if needsHumanReview itself is false", async () => {
    ...
    const thread = await recordAndClassifyUnmatchedSms(phone, "what does genesis health offer?");
    expect(thread.status).toBe("needs_review");
