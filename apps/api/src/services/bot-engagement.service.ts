...
/**
 * Whether a lead's purchase followed at least one real reply to Chris — SMS
 ...
 * always means the customer wrote it (never Chris's own text), so this is a
 * genuine two-way exchange, not just "a message was sent to them." Mia's
 ...
 */
const spokeToChrisBeforePurchaseSql = sql<boolean>`exists (
 ...
 * splits them into "replied to Chris before buying" vs "never did," and
...
export async function getBotEngagementSummary(query: CustomersSummaryQuery): Promise<BotEngagementSummary> {
  ...
  const rows = await db
    .select({
      spokeToBot: spokeToChrisBeforePurchaseSql,
