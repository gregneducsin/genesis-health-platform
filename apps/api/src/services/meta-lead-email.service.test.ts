...
describe("sweepMetaLeadEmailTriggers", () => {
  ...
  it("sends the correct template per step (opener vs plan_comparison have different subjects)", async () => {
    ...
    expect(subjects).toContain("Your Genesis Health visit is waiting — don't lose your spot");
    expect(subjects).toContain("Which Genesis Health plan is right for you?");
  });
});
