 ...
 * for a name AND an email before it creates anything (once that pipeline
 * exists here): an email inquiry always comes with a working email address
 * (that's how the sender is reaching us), but never a phone, so
 * collectedPhone on the thread plays the equivalent role — gathered
...
function buildTranscript(messages: readonly UnmatchedEmailMessage[]): string {
  ...
  const lines = capped.map((m) => `${m.direction === "inbound" ? "Sender" : "Genesis Health"} (Subject: ${m.subject}): ${m.body}`);
  let text = lines.join("\n\n");
...
function systemPrompt(candidates: readonly MatchCandidate[], fromNameHeader: string | null, knownPhone: string | null): string {
  const candidateList = candidates.length
    ? candidates.map((c, i) => `${i}: ${c.firstName} ${c.lastName} (${c.email})`).join("\n")
    : "(no plausible candidates found)";

  return `You triage inbound email at Genesis Health, a healthcare company, for a sender whose email address doesn't match any customer record in the CRM. You're seeing the full thread so far with this sender, not just one message.
...
- If we don't know the sender's real personal name yet, the reply MUST ask for it naturally (e.g. "Could you share your name so we can look into this for you?") — asking for their name takes priority over any other clarifying question, including a product/pricing question they may have already asked.
- If we know their real personal name but not their phone number, the reply MUST ask for their phone number instead, framed as needing it to get an account started or to look into things for them (e.g. "Thanks! Before I go over pricing or product details, let me get an account started for you — what's a good phone number for you?") — never ask for both name and phone in the same message, and still don't answer their product/pricing question yet even though you now know their name.
...
async function classifyAndDraft(
  ...
  const createPromise = client.messages.create({
    model: MODEL,
    max_tokens: 500,
    system: systemPrompt(candidates, fromNameHeader, knownPhone),
    tools: [CLASSIFY_TOOL],
...
const ACK_ASKING_NAME_VARIANTS = [
  "Thanks for reaching out to Genesis Health — could you share your name so we can help you further? A member of our team will follow up shortly.",
  "Thanks for getting in touch with Genesis Health — could you let us know your name so we can help you out? Our team will follow up shortly.",
] as const;
const ACK_KNOWN_NAME_VARIANTS = [
  "Thanks for reaching out to Genesis Health — a member of our team will follow up shortly.",
  "Thanks for getting in touch with Genesis Health — our team will follow up with you shortly.",
] as const;
...
export async function recordAndClassifyUnmatchedEmail(input: {
  ...
  try {
    classification = await classifyAndDraft(input.fromAddress, fromNameHeader, thread.collectedPhone, messages, candidates);
  } catch (err) {
  ...
  if (leadResult?.justCreated) {
    ...
    // Claude, so default to acknowledging. Uses the CONFIRMED name
    // (classification.senderName), not the raw header — so a display name
    // like "Ark Health" correctly triggers the "what's your name?" ack
    // instead of wrongly addressing them as "Ark".
    await sendAutoAcknowledgment(thread.id, input.fromAddress, input.subject, classification?.senderName ?? null, input.messageId, thread.receivingAddress);
...
export async function sendUnmatchedInboundEmailReply(id: string, body: string): Promise<UnmatchedEmailReplyResult> {
  ...
  const lastSubject = lastMessage?.subject ?? "Your message to Genesis Health";
