...
describe("handleIbluSendWebhook", () => {
  ...
  it("routes to the purchased customer's Mia conversation, not a stale unsold lead sharing the same phone", async () => {
    processInboundMessageMock.mockClear();
    processInboundSupportMessageMock.mockClear();

    const phone = uniquePhone();
    // The stale lead: signed up first, never purchased, only ever talked to Chris.
    const staleLeadId = await seedCustomer(phone);
    await db.insert(conversationsTable).values({ personId: staleLeadId });
    // The real customer: same phone, purchased later, has a Mia support conversation.
  ...
  it("routes to Chris for a known customer's first-ever text, with no prior Chris or Mia conversation", async () => {
    ...
    const phone = uniquePhone();
    const personId = await seedCustomer(phone);
    await handleIbluSendWebhook(envelope({ data: { phone_number: phone, content: "hey is this genesis health" } }));

    expect(processInboundMessageMock).toHaveBeenCalledWith(personId, "hey is this genesis health");
    expect(processInboundSupportMessageMock).not.toHaveBeenCalled();
  });
