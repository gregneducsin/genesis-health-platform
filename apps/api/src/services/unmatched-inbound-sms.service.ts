  ...
  input_schema: {
    ...
    properties: {
      ...
      productCategoryMentioned: {
        ...
        description:
          "Does suggestedReply describe, imply, or reference what Genesis Health offers, does, or sells? 'weight_loss_medication' if it references the one real product (prescription semaglutide/tirzepatide via intake questionnaire). 'other_business_line' if it references or implies ANY other service, product category, or business line — software, platforms, patient engagement, practice management, consulting, staffing, or anything else — set this even if you're not fully sure it's inaccurate, since Genesis Health doesn't offer anything beyond the medication program. 'none' if the reply doesn't describe what Genesis Health offers at all (e.g. just asking for a name or email). Answer honestly — this is checked independently of the reply text itself.",
      },
    },
    required: [
      "intent",
...
function buildTranscript(messages: readonly UnmatchedSmsMessage[]): string {
  ...
  const lines = capped.map((m) => `${m.direction === "inbound" ? "Sender" : "Genesis Health"}: ${m.body}`);
  let text = lines.join("\n");
...
function systemPrompt(candidates: readonly MatchCandidate[], knownName: string | null, knownEmail: string | null, pendingConfirmation: boolean): string {
  ...
  return `You triage inbound SMS at Genesis Health, a healthcare company, for a phone number that doesn't match any customer record in the CRM. You're seeing the full text thread so far with this sender, not just one message.

Genesis Health's actual product is prescription weight-loss medication —
...
describe when asked what Genesis Health offers, does, or sells. A real
customer was once told a sibling company offers "digital health platforms,
patient engagement tools, and practice management services" — none of
which are real; that got invented because nothing told the model what the
business actually does. Never invent services, product categories, or
business lines beyond the one above. If asked about something outside this
(e.g. peptides, other medications, anything you're not sure Genesis
offers), don't confirm or deny it either way — stay non-committal and keep
moving the conversation forward through the normal flow (name/email/handoff)
instead of answering with a guess. Set productCategoryMentioned honestly on
every turn, even when you're confident suggestedReply is fine — it's
checked independently of the reply text itself.
...
const ACK_VARIANTS = [
  "Hey! Thanks for reaching out to Genesis Health — could you share your name so I know who I'm chatting with? Our team will follow up shortly.",
  "Hi there, thanks for texting Genesis Health! What's your name so I know who I'm talking to? We'll follow up with you shortly.",
] as const;
