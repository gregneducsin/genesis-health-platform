...
describe("listWebhookEvents", () => {
  ...
  it("filters by source", async () => {
    ...
    const shippedId = await seedWebhookEvent({ source: "bask_order_shipped" });

    const items = await listWebhookEvents({ source: "bask_order" });
    expect(items.some((i) => i.id === orderId)).toBe(true);
    expect(items.some((i) => i.id === shippedId)).toBe(false);
  });
