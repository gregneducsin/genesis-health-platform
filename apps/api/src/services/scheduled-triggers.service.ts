...
/**
 * Review-request arming is disabled for Genesis Health (no write-a-review
 * link to offer yet — see order-fulfillment.service.ts's handleOrderShipped
 * and index.ts's sweep registration comment), so this table is expected to
 * always be empty here. Kept in the check anyway rather than special-cased
 * out: harmless (one extra empty-result query) and it starts working the
 * moment arming is re-enabled, with nothing else to remember to update.
 */
