...
describe("recordAndClassifyUnmatchedEmail", () => {
  ...
  it("creates a new lead once the sender's real personal name AND phone number are known and Claude classifies genuine new-lead interest", async () => {
    createMock.mockResolvedValueOnce(
      toolResponse(
        classification({
          intent: "new_lead_interest",
  ...
  it("does not create a lead when the extracted phone number doesn't look like a real one", async () => {
    createMock.mockResolvedValueOnce(toolResponse(classification({ intent: "new_lead_interest", senderName: "Jamie Lee", senderPhone: "call me" })));
    const thread = await recordAndClassifyUnmatchedEmail({
...
describe("auto-acknowledgment", () => {
  ...
  it("sends the name-known variant (no request for a name) when Claude confirms the sender's real personal name", async () => {
    createMock.mockResolvedValueOnce(toolResponse(classification({ senderName: "Casey Nguyen" })));
    sendEmailMock.mockClear();
    ...
    await recordAndClassifyUnmatchedEmail({
      ...
      subject: "Question",
      body: "Hi, quick question. — Casey Nguyen",
      messageId: null,
    });
    ...
    expect(html.toLowerCase()).toMatch(/thanks for (reaching out to|getting in touch with) genesis health/);
  });
  ...
  it("sends Claude's own drafted reply (not a repeat of the fixed ack) on a second message, since replies are auto-sent by default now", async () => {
    ...
    createMock.mockResolvedValueOnce(toolResponse(classification({ summary: "Second.", senderName: "Jordan", suggestedReply: "Thanks Jordan! What's your phone number?" })));
    sendEmailMock.mockClear();
    ...
    expect(sendEmailMock).toHaveBeenCalledTimes(1);
    const [, , html] = sendEmailMock.mock.calls[0];
    expect(html).toContain("Thanks Jordan! What's your phone number?");
    expect(thread.status).toBe("replied");
    expect(thread.suggestedReply).toBeNull();
  });
